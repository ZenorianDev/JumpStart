    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-900 text-white p-6">
      <form onSubmit={onSubmit} className="w-full max-w-md bg-white/5 p-8 rounded">
        ...
        <label className="block">Name
          <input required type="text" value={name} onChange={e=>setName(e.target.value)} className="w-full p-2 mt-1 rounded bg-black/50" />
        </label>
        ...
        <button className="mt-6 w-full px-4 py-2 bg-indigo-600 rounded">Sign up</button>
      </form>
    </div>
  )
}
